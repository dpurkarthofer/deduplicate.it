...
# ── CONFIGURATION ──────────────────────────────────────────────────────────────
# Place all input files in a 'source/' folder in the current working directory,
# or point --source somewhere else.
# All supported formats (.ris, .txt, .nbib, .bib, .csv, .tsv, .ciw, .enw)
# are auto-detected. Files are processed in alphabetical order.

__version__ = '1.2.0'

SOURCE_DIR = Path('source')        # relative to the current working directory
...
def _find_file(name: str) -> str:
    """Locate a template: packaged data first, then script-relative, then CWD."""
    try:
        from importlib.resources import files
        # One component per joinpath() call: Traversable.joinpath() only began
        # accepting several on Python 3.11.
        p = files('deduplicate_it').joinpath('templates').joinpath(name)
        if p.is_file():
            return str(p)
    except Exception:
        pass
...
def _parse_args(argv=None):
    ...
    p = argparse.ArgumentParser(
        prog='deduplicate-it',
        description='Remove duplicate records from literature search exports '
                    '(deterministic DOI + normalised-title matching).')
    p.add_argument('-s', '--source', type=Path, default=Path('source'),
                   help='folder holding the export files (default: ./source)')
